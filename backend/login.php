...
try {
    ...
    // Debugging: Log received username and password (Avoid logging passwords in production)
    error_log("Username: " . $usernameInput);
    error_log("Password: " . $passwordInput);
    ...
    if ($user && password_verify($passwordInput, $user['password'])) {
        if ($user['type'] == 2) {
            ...
            echo json_encode(['success' => false, 'error' => 'Only staff can log in.']);
        }
    } else {
        // Invalid credentials
